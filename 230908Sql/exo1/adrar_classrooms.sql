DROP DATABASE adrar_classrooms;
CREATE DATABASE  adrar_classrooms;
...
SELECT MIN(note) AS 'plus basse note', MAX(note) AS 'plus haute note' 
FROM EVALUER;
	
#Quelles sont les moyennes de chaque étudiant dans chacune des matières ?
SELECT E.id_etud, M.libelle_matiere, AVG(EV.note) AS MoyEtuMat 
FROM EVALUER EV, MATIERES M, ETUDIANTS E 
WHERE EV.code_matiere = M.code_matiere 
AND EV.id_etud = E.id_etud 
GROUP BY E.id_etud, M.libelle_matiere;

#Quelles sont les moyennes par matière ? Avec la vue MGETU de la question 3 (MoyEtuMat)
SELECT libelle_matiere, AVG(MoyEtuMat) 
FROM MoyEtuMat 
GROUP BY libelle_matiere;

#Quelle est la moyenne générale de chaque étudiant ? Avec la vue MGETU de la question 3 (MoyEtuMat)
SELECT id_etud, SUM(MoyEtuMat*coeff_matiere)/SUM(coeff_matiere) AS MgEtu
FROM MOYETUMAT 
GROUP BY id_etud;
...
SELECT id_etud, nom_etud, prenom_etud, MgEtu 
FROM MGETU
WHERE MgEtu >= (
SELECT AVG(MgEtu) 
FROM MGETU
